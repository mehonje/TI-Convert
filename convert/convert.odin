...
Hex_Decoding_Error :: struct {
	str: string,
}

Name_Too_Long :: struct {
	size:     int,
	expected: int,
}

Comment_Too_Long :: struct {
	size:     int,
	expected: int,
}
...
eightxp_to_txt :: proc(from_path, to_path: string, debug: bool) -> Conversion_Error {
	...
	if len(byte_data) < 76 {
		return File_Size_Too_Small{size = len(byte_data), expected = 76}
	}
...
txt_to_eightxp :: proc(from_path, to_path: string) -> Conversion_Error {
	...
	if len(meta[0]) > 8 {
		return Name_Too_Long{size = len(meta[0]), expected = 8}
	}
	if len(meta[1]) > 42 {
		return Comment_Too_Long{size = len(meta[1]), expected = 42}
	}
	...
	{ 	// append file_type
		...
		if !ok {
			return Hex_Decoding_Error{str = meta[2]}
		}
	...
	{ 	// append is_archived
		...
		if !ok {
			return Hex_Decoding_Error{str = meta[3]}
		}
